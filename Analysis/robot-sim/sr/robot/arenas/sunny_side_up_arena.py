...
class SunnySideUpArena(Arena):
	...
	for i in range(1,10):
		...
		while abs(x-7)<=0.3 or abs(x-9)<=0.3 or abs(x+7)<=0.3 or abs(x+9)<=0.3 or  abs(x+3.5)<=0.3 or abs(x+1.5)<=0.3 or abs(x-3.5)<=0.3 or abs(x-1.5)<=0.3:
			x = random.uniform(-9,9)

		while abs(y-3)<=0.3 or  abs(y-5)<=0.3 or abs(y+3)<=0.3 or  abs(y+5)<=0.3 or abs(y+4.75)<=0.3 or abs(y+1.25)<=0.3 or abs(y-0.75)<=0.3:
			y = random.uniform(-4.5,4.5)
       	 	token.location = (x,y)
        	self.objects.append(token) 
      		count+1
